from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QComboBox, QPushButton, QMessageBox, QSizePolicy, QCheckBox, QFrame
from PyQt5.QtGui import QFont
import sqlite3

class ToolTransferDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Transfer Worker Tool Data")
        self.setFixedSize(900, 80)  # Fixed window size
    ...
    def onTransfer(self):
        ...
        # Retrieve current tab information from parent
        parentTabIndex = self.parent().tabWidget.currentIndex()
        parentTabText = self.parent().tabWidget.tabText(parentTabIndex)
        
        # Retrieve primary key data from UI elements in the parent window
        ...
        # Determine tool_id based on the parent tab index
        if parentTabIndex == 0:
            parentToolID = "LiFFT"
        elif parentTabIndex == 1:
            parentToolID = "DUET"
        elif parentTabIndex == 2:
            parentToolID = "ST"
        else:
            parentToolID = ""  # Default case if needed

        
        # Retrieve selected values from the transfer window
        ...
        # Check if the selected destination is the same as the current one
        if (
            plant_name == parentPlantName and
            section_name == parentSectionName and
            line_name == parentLineName and
            station_id == parentStationID and
            shift_id == parentShiftID
        ):
            QMessageBox.warning(self, "Invalid Transfer", "Cannot transfer tool data to the same location.")
            return

        
        # **Check if transfer is possible**
        if self.checkIfTransferLocationExists(parentWorkerID, plant_name, section_name, line_name, station_id, shift_id, parentToolID):
            QMessageBox.warning(self, "Transfer Not Allowed", "Tool data already exists at the target destination.")
        ...
        try:
            ...
            # **Retrieve source record from WorkerStationShiftErgoTool**
            cursor.execute('''
            ...
            ''', (parentWorkerID, parentPlantName, parentSectionName, parentLineName, parentStationID, parentShiftID, parentToolID))
    
            source_record = cursor.fetchone()
    
            if not source_record:
                QMessageBox.warning(self, "Transfer Error", "No data found to transfer.")
                conn.close()
                return
    
            # **Prepare values for the new entry (duplicate with new location keys)**
            columns = [desc[0] for desc in cursor.description]  # Extract column names from the query result
            values = list(source_record)  # Convert tuple to list for modification
    
            # **Modify primary key values for new destination**
            ...
            # **Duplicate records from LifftResults**
            if parentToolID == "LiFFT":
                cursor.execute('''
                ...
                if lifft_records:
                    lifft_columns = [desc[0] for desc in cursor.description]  # Extract column names
            ...
            # **Duplicate records from DuetResults**
            if parentToolID == "DUET":
                cursor.execute('''
                ...
                if duet_records:
                    duet_columns = [desc[0] for desc in cursor.description]  # Extract column names
                    duet_insert_query = f'''
            ...
            # **Duplicate records from TstResults**
            if parentToolID == "ST":
                cursor.execute('''
                ...
                if tst_records:
                    tst_columns = [desc[0] for desc in cursor.description]  # Extract column names
                    tst_insert_query = f'''
            ...
            # **Check if "Copy Data" is NOT checked before deleting**
            if not self.copy_data_checkbox.isChecked():
                # **Enable Foreign Key Constraints**
                cursor.execute("PRAGMA foreign_keys = ON;")
            
                # **Delete existing entry in WorkerStationShiftErgoTool (Cascade deletes in child tables)**
                delete_query = '''DELETE FROM WorkerStationShiftErgoTool 
                                  WHERE worker_id=? AND plant_name=? AND section_name=? 
                                  AND line_name=? AND station_id=? AND shift_id=? AND tool_id=?'''
                cursor.execute(delete_query, (parentWorkerID, parentPlantName, parentSectionName, 
                                              parentLineName, parentStationID, parentShiftID, parentToolID))
            
                conn.commit()  # Apply deletion
            ...
            QMessageBox.information(self, "Transfer Successful", "Tool data successfully transferred.")
            self.close()  # Close the transfer window
